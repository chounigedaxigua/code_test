...
#define CONFIG_FILE "/home/xhh/wifi/wifi_tcp/config.ini"
typedef enum 
{
  STATE_OK,
  STATE_FAIL,
...
STATE _ssid_info_scan(ssid_info_t *p_info,int08_t *p_scan_key)
{
  ...
  sprintf( scan_str,"sudo iwlist wlan0 scanning | grep ESSID |grep \"%s\" | awk -F':' '{print $2}' 2>>  /etc/init.d/err.log",p_scan_key);
